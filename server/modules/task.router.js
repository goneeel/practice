...
router.get('/', (req, res) => {
  ...
  pool.query(sqlText)
    ...
    .catch( (err) => {
      console.log('Ошибка получения всех задач: ', err);
      res.sendStatus(500);
    })
});
...
router.get('/:id', (req, res) => {
  ...
  pool.query(sqlText, [taskId])
    ...
    .catch( (err) => {
      console.log(`Ошибка получения задачи с id ${taskId}:`, err);
      res.sendStatus(500);
    })
});
...
router.post('/', (req, res) => {
  ...
  if (task.description == null) {
    res.sendStatus(500);
    console.log('Ошибка добавления задачи, требуется описание.');
    return;
  }

  if (task.category_id == null) {
    res.sendStatus(500);
    console.log('Ошибка добавления задачи, требуется категория.');
    return;
  }
...
router.delete('/:id', (req, res) => {
  ...
  .catch( (err) => {
    console.log(`Ошибка удаления категории с id=${taskId}`, err);
    res.sendStatus(500);
  })
});
...
router.put('/:id', (req, res) => {
  ...
  console.log('Обновление запроса:', queryStuff.sqlText);
  console.log('Обновление значений запроса:', queryStuff.values);
